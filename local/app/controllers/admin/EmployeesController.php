...
class EmployeesController extends \AdminBaseController {
	...
	public function create()
	{
		...
		$this->data['new'] = Employee::orderBy('employeeID', 'desc')->first();
		if( $this->data['new'] ) $this->data['new']->employeeID = sprintf('%05d', $this->data['new']->employeeID + 1);
	...
	public function store()
	{
		...
		try {
			...
			if ($input['daily_rate'] != '')
			{
				Salary::create([
					'employeeID' 	=> $input['employeeID'],
					'employeeName' 	=> ucwords(strtolower($input['fullName'])),
					//'type'       	=> 'contractual',
					'department'    => $input['department'],
					'daily_rate'    => $input['daily_rate'],
					'hour_rate'     => $input['daily_rate'] / 8,
					'overtime'      => 0,
					'salary'       	=> '',
					'remarks'    	=> ''
	...
	public function update($id)
	{
		...
		else if(Input::get('updateType')=='company')
		{
			...
			if ($validator->fails())
			{
				...
				$company_details->department = Input::get('department');
				//$company_details->designation = Input::get('designation');
				$company_details->designation = '1';
			...
			DB::table('salary')
                    ->where('employeeID', $id)
                    ->update(array('department' => $input['department'],
                    				'daily_rate' => $input['salary'], 
                    				'hour_rate' => $input['salary'] / 8));
		}
		...
		else if(Input::get('updateType')=='personalInfo')
		{
			...
			DB::table('salary')
                    ->where('employeeID', $id)
                    ->update(array('employeeName' => ucwords(strtolower($input['fullName']))));
