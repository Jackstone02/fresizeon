 ...
 */
	public function create()
	{
        ...
        $department       = (Input::get('department') != '') ? Input::get('department') : '1';
        $date             = date('Y-m-d', strtotime($date));

        $attendance_count           = Attendance::where('date','=',$date)->count();
        //$employee_count             = Employee::where('status','=','active')->count();
        $employee_count = count(DB::select("select * from employees where status = 'active' AND joiningDate <= '". $date ."'"));

        
        //if( $employee_count  ==  $attendance_count ) {
        if( $employee_count != 0 AND $employee_count  ==  $attendance_count ) {
	...
	public function edit($date)
	{
        ...
        //$this->data['employees']    =   Employee::where('status','=','active')->get();
        $this->data['employees'] = DB::select("select * from employees where status = 'active' AND joiningDate <= '". $date ."'");
	...
	public function update($date)
    {
        ...
        foreach ($input['employees'] as $employeeID)
        {
			...
			if($user->application_status !='approved' || ($user->application_status =='approved' && isset($input['checkbox'][$employeeID])=='on'))
			{
                ...
                $update->department     = (isset($input['checkbox'][$employeeID])=='on')?$input['department'][$employeeID]:null;
                $update->time_in        = (isset($input['checkbox'][$employeeID])=='on')?$input['time_in'][$employeeID]:null;
                $update->time_out       = (isset($input['checkbox'][$employeeID])=='on')?$input['time_out'][$employeeID]:null;
                $update->work_hours     = (isset($input['checkbox'][$employeeID])=='on')?$input['work_hours'][$employeeID]:null;
                $update->overtime       = (isset($input['checkbox'][$employeeID])=='on')?$input['overtime'][$employeeID]:null;
                $update->leaveType      = (isset($input['checkbox'][$employeeID])=='on')?null:'sick';
            ...
            $days_work = count(DB::select( DB::raw("select * from attendance where employeeID = '". $employeeID ."' AND status='present'")));       

            DB::table('salary')
                ->where('employeeID',$employeeID)
                ->update(array('days_work' => $days_work));

            $latest_dept = DB::select("select * from attendance where employeeID = '". $employeeID ."' AND status='present' ORDER by date DESC LIMIT 1");


            if( $update->department != null ) {
                DB::table('employees')
                    ->where('employeeID',$employeeID)
                    //->update(array('department' => $update->department));
                    ->update(array('department' => $latest_dept[0]->department));
